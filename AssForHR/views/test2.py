...
def loginInSystem(requestInfo):
    permanentID = getpermanentID(rooturl=requestInfo.rooturl, headers=requestInfo.headers,
                                 loginurl=requestInfo.loginurl)
    postData = {'account': requestInfo.webname, 'password': requestInfo.webpsw, 'cultureCode': 'zh-CN',
                'permanentId': permanentID}
    print('requestInfo.loginurl:%s' % (requestInfo.loginurl))
    response = requestNeedurl(needurl=requestInfo.loginurl, postData=postData, headers=requestInfo.headers,
...
class UrlRequest:
    def __init__(self, rooturl, host, port, webname, webpsw, tmpFilePath, showMode, cookie, headers, loginurl,
                 CookieOpener, http):
...
def PublicGroup(requestInfo):
    ...
    listPublicGroupName = listPublicGroupName + [singleGroup.find_all('span')[2].string for singleGroup in
                                                 readSoup.find('div', id='M_UWebTreePublicCode_1').find_all('div') if
                                                 len(singleGroup.find_all('span')[2].string) > 0].copy()
    ...
    for i in range(totalPagesNum):
        if i == 0 and int(recordTotalNum) > 10 and totalPagesNum == 1:
            ...
            driver.find_element_by_xpath(
                "//div[@id='grdNavigator_PagerContainer']/div/ul/li[" + str(int(i) + 1) + "]/a/span").click()
            time.sleep(2)
            tmpPageSource = driver.page_source
            readSoup = bs.BeautifulSoup(tmpPageSource, 'lxml')
    ...
    if len(extendValuelist) > 0:
        for singleExtendValue in extendValuelist:
            ...
            for i in range(len(singleExtendValue)):
                if extendNamelist[i] is not None and len(extendNamelist[i]) > 0 and extendNamelist[i] in ['群组名称',
                                                                                                          '业务数据群组类型',
                                                                                                          '群组描述',
                                                                                                          '群组编号']:
...
def queryExtendDetail(driver, requestInfo, groupID):
    ...
    extendDetailNamelist = [singletd.find('nobr').string for singletd in singletr.find_all('th') if
                            singletd.find('nobr') is not None].copy()
    for singletr in readSoup.find('tbody', class_='ig_e531842b_r1 WebGridText ig_e531842b_r4').find_all('tr'):
    ...
    if len(extendDetailValuelist) > 0:
        for singleExtendDetailValue in extendDetailValuelist:
            ...
            for i in range(len(singleExtendDetailValue)):
                if extendDetailNamelist[i] is not None and len(extendDetailNamelist[i]) > 0 and extendDetailNamelist[
                    i] in ['序号', '字段名称', '数据类型', '是否必填', '显示方式', '数据关联', '字段编号']:
...
def addExtendGroup(driver, requestInfo, addSingleExtendGroup, listExtendGroup):
    ...
    if addExtendGroupName in listExtendGroupName:
        ...
        if addExtendGroupRepeatLimit in ['无限制', '按日', '按月', '按季度', '按年']:
            driver.find_element_by_xpath(
                "//select[@id='DropDownListLimitType']/option[@title='" + addExtendGroupRepeatLimit + "']").click()
        else:
...
def addExtendDetail(driver, requestInfo, addSingleExtendDetail, listExtendDetail, groupID):
    ...
    if addExtendDetailName in listExtendDetailName:
        ...
        if addExtendDetailShowMode == '选择框' and addExtendDetailPublicCode is not None and len(
                addExtendDetailPublicCode) > 0:
            driver.find_element_by_xpath(
                "//select[@id='drpPublicCode']/option[@title='" + addExtendDetailPublicCode + "']").click()
        time.sleep(2)
        driver.find_element_by_id('btnAdd').click()
        time.sleep(2)
        try:
...
def importPublicCodeDetail(driver, requestInfo, listExtendByDoc, listPublicByDoc):
    ...
    pc_ExtendGroup = {'repeatLimit': '无限制', 'extendName': '更新公用代码明细',
                      'extendDesc': 'Sys config for update publiccodeitem', 'extendType': '组织业务数据',
                      'codeItem': [{'列字段名称': '公用代码类型', '显示方式': '文本框', '数据类型': '文本', '是否必填': '否', '数据关联': '', '序号': '1'},
                                   {'列字段名称': '公用代码编号', '显示方式': '文本框', '数据类型': '文本', '是否必填': '否', '数据关联': '', '序号': '2'},
                                   {'列字段名称': '公用代码名称', '显示方式': '文本框', '数据类型': '文本', '是否必填': '否', '数据关联': '',
                                    '序号': '3'}]}
    # firt step:add extend group
    addExtendGroup(driver, requestInfo, pc_ExtendGroup, listExtendGroup)
    # second step:add extend detail
    listExtendGroup = queryExtendGroup(driver, requestInfo)
    pc_SysExtendGroup = [singleExtendGroup for singleExtendGroup in listExtendGroup if
                         singleExtendGroup.get('群组名称') == pc_ExtendGroup.get('extendName')]
    ...
    if pc_ExtendDetail is not None and len(pc_ExtendDetail) > 0:
        for single_pc_ExtendDetail in pc_ExtendDetail:
            addExtendDetail(driver, requestInfo, single_pc_ExtendDetail, pc_SysExtendGroup_ExtendDetail,
                            pc_SysExtendGroup_GroupID)
    ...
    time.sleep(2)
    driver.find_element_by_xpath(
        "//select[@id='drpGroup']/option[@title='" + pc_ExtendGroup.get('extendName') + "']").click()
    time.sleep(2)
...
def updateExtendGroup(driver, requestInfo, listExtendByDoc):
    ...
    for cur_ExtendGroup in listExtendByDoc:
        ...
        cur_SimExtendGroup = [singleExtendGroup for singleExtendGroup in listExtendGroup if
                              singleExtendGroup.get('群组名称') == cur_ExtendGroup.get('extendName')]
        ...
        if cur_ExtendDetail is not None and len(cur_ExtendDetail) > 0:
            for sin_cur_ExtendDetail in cur_ExtendDetail:
                addExtendDetail(driver, requestInfo, sin_cur_ExtendDetail, cur_ExtendGroup_ExtendDetail,
                                cur_ExtendGroup_GroupID)
...
def addInsArea_Post(requestInfo, sinInsArea):
    if sinInsArea is None or len(sinInsArea) == 0:
        ...
        response = requestNeedurl(needurl, postData=needPostData, headers=requestInfo.headers,
                                  CookieOpener=requestInfo.CookieOpener)
...
def queryInsArea(requestInfo):
    ...
    content = requestInfo.http.request(uri=needurl, method='GET', headers=requestInfo.headers)[1]
    readSoup = bs.BeautifulSoup(content.decode(), 'lxml')
    listInsArea = listInsArea + [singleGroup.find_all('span')[2].string for singleGroup in
                                 readSoup.find('div', id='M_TreeInsureArea_1').find_all('div') if
                                 len(singleGroup.find_all('span')[2].string) > 0].copy()
...
def queryPersonPropID(requestInfo):
    ...
    content = requestInfo.http.request(uri=needurl, method='GET', headers=requestInfo.headers)[1]
    readSoup = bs.BeautifulSoup(content.decode(), 'lxml')
    listvalue = {sinoption.string: sinoption.get('value') for sinoption in
                 readSoup.find('select', id='UcStaffSelect1_ddlqins').find_all('option') if
                 sinoption.string is not None}
...
def queryInsTypeID(requestInfo):
    ...
    readSoup = bs.BeautifulSoup(content.decode(), 'lxml')
    listvalue = {sinoption.string: sinoption.get('value') for sinoption in
                 readSoup.find('select', id='DdlInsuranceType').find_all('option') if sinoption.string is not None}
    print('listvalue:%s' % (listvalue))
    return listvalue
...
def readInsAreaDetailByDoc(requestInfo, filename):
    ...
    tableContent = [{content_Name[i]: sin_rowvalue[i] for i in range(len(sin_rowvalue))} for sin_rowvalue in
                    content_value]
...
def getCalEmpInfo(driver, requestInfo, calempid):
    ...
    needurl = requestInfo.rooturl + r'/ePayroll/PersonalPayrollInformationManage/PayrollStructGroupInputEdit.aspx?optype=Edit&strSubjectID=&userid=' + personid + r'&AREAID=19'
    js = "window.open('" + needurl + "')"
    driver.execute_script(js)
    time.sleep(2)

    needurl = requestInfo.rooturl + r'/ePayroll/PersonalPayrollInformationManage/PersonAddReduceSingleManage.aspx?viewtype=tab'
    js = "window.open('" + needurl + "')"
    driver.execute_script(js)
    time.sleep(2)
    ...
    needurl = requestInfo.rooturl + r'/ePayroll/PersonalPayrollInformationManage/PersonalBusinessDataImport.aspx'
    js = "window.open('" + needurl + "')"
    driver.execute_script(js)
    time.sleep(2)

    needurl = requestInfo.rooturl + r'/ePayroll/PersonalPayrollInformationManage/PersonalPayrollRule.aspx'
    js = "window.open('" + needurl + "')"
    driver.execute_script(js)
    time.sleep(2)
...
def getCalList(driver, requestInfo):
    ...
    if len(extendValuelist) > 0:
        for singleExtendValue in extendValuelist:
            singleExtendGroup = {}
            for i in range(len(singleExtendValue)):
                if extendNamelist[i] is not None and len(extendNamelist[i]) > 0 and extendNamelist[i] in ['薪资发放名称',
                                                                                                          '编号',
                                                                                                          '薪资发放状态']:
                    singleExtendGroup[extendNamelist[i]] = singleExtendValue[i]
    ...
    if packageID == '':
        ...
        if workNO == '':
            ...
            if viewState == '':
                ...
                needPostData = {'__VIEWSTATE': viewState, 'txtWorkNO': workNO, 'BtnDebug': '调试',
                                'TxtPackageID': packageID}
                response = requestNeedurl(needurl, postData=needPostData, headers=requestInfo.headers,
                                          CookieOpener=requestInfo.CookieOpener)
...
def getQCModuleID(requestInfo):
    needurl = requestInfo.rooturl + r'/eQueryCenter/BaseConfig/ModuleManagement.aspx'
    response, content = requestInfo.http.request(needurl, 'GET', requestInfo.headers)
    readSoup = bs.BeautifulSoup(content.decode('utf-8'), 'lxml')
    print(readSoup)
    '''
    listGroup = []
    singletr = readSoup.find('thead',class_='ig_71cf6462_r1 WebGridText ig_71cf6462_r4').find('tr')
    ...
    if  len(extendValuelist) > 0: 
        ...
        print('No extend group list') 
    print('listExtendGroup:%s'%(listGroup))
    '''
...
def web_GAIAPAYROLLCAL(request):
    ...
    return render_to_response('GURU/GAIAPAYROLLCAL.html', {'defaultcalempno': json.dumps(defaultcalempno),
                                                           'defaultcalpackagename': json.dumps(defaultcalpackagename)})


def GAIAPAYROLL(request):
    '''request for payroll '''
    # defaultFileName = r'E:\工作文档\1608-中银\06业务流程分析\薪资管理\test.docx'
    defaultFileName = r'E:\工作文档\1608-中银\06业务流程分析\薪资管理\中银保险薪资管理系统_薪资管理_设计_V1.1_170617.docx'
    ...
    return render_to_response('GURU/GAIAPAYROLL.html', {
        ...
        'defaultINSfilename': json.dumps(defaultINSfilename)
    })
...
def web_updatePublicGroupByDoc(request):
    if request.method == "POST":
        ...
        servername = dbSourceInfo.getServername()
        dbname = dbSourceInfo.getDbname()
        dbusername = dbSourceInfo.getDbusername()
        dbpsw = dbSourceInfo.getDbpsw()
        ...
        http = httplib2.Http()
        requestInfo = UrlRequest(rooturl=rooturl, host=host, port=port, webname=webname, webpsw=webpsw,
                                 tmpFilePath=tmpFilePath, showMode=showMode, cookie=cookie, headers=headers,
                                 loginurl=loginurl, CookieOpener=CookieOpener, http=http)
...
def web_payrollCal(request):
    ...
    if action == '点击查看人事资料':
        resLogin = loginInSystem(requestInfo)
        driver = createDriver(requestInfo)
        getCalEmpInfo(driver, requestInfo, calempno)
